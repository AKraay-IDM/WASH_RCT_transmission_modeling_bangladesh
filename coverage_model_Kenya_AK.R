library(dplyr) # for 'n_distinct' function
library(deSolve)
...
data$adh_cat= array(NA,nrow(data))
...
for (i in 1:7){
  S_prev[i,1] = sum(data[data$armid==i & data$svy==0,"imp_lat"])/length(data[data$armid==i & data$svy==0,"imp_lat"])
  S_prev[i,2] = sum(data[data$armid==i & data$svy%in%c(1,2),"imp_lat"])/length(data[data$armid==i & data$svy%in%c(1,2),"imp_lat"])
}
...
ggsave(file="/Users/aliciakraay/Dropbox/WASHBenefits_Kenya/Fig_adherence.pdf", width = 7, height = 3.5)
...
for (j in 0:2){
  for (i in 1:length(clusters)){
    ...
    data[data$svy==j & data$clusterid == clusters[i],"cluster_imp_lat"]=mean(data[data$svy==j & data$clusterid == clusters[i],"imp_lat"],na.rm=T)
...
tic()
sample_and_NLL = parSapply(cluster,1:5000,FUN=compute_NLL_function)
toc()
